...
# Shapes reduced to fit in VRAM during profiling
SHAPES = [
    # (M,    K,     N,     label)
    (128, 3072, 3072, "attn_proj  (128 tok)"),
    (256, 3072, 3072, "attn_proj  (256 tok)"),
    (512, 3072, 3072, "attn_proj  (512 tok)"),
    (1024, 3072, 3072, "attn_proj  (1k tok)"),
    (128, 3072, 12288, "mlp_up     (128 tok)"),
    (256, 3072, 12288, "mlp_up     (256 tok)"),
    (128, 12288, 3072, "mlp_down   (128 tok)"),
...
def bench_fp8_hybrid(fp8_lin: Fp8AmpereLinear, x_bf16: torch.Tensor) -> dict:
    """Fp8AmpereLinear.forward() — hybrid path (kernel vs cuBLAS)."""

    def fn():
        return fp8_lin(x_bf16)
...
def run(csv_mode: bool = False):
    ...
    print(f"GPU: {props.name}  SM {props.major}.{props.minor}  VRAM {props.total_memory // 1024**3} GiB")

    if not _is_ampere_only(device):
        print(
            "WARNING: Not an Ampere GPU — IMMA path will not be used by the module, but the benchmark will still run the kernel directly."
        )
    ...
    sep = "-" * 108
    header = f"{'Shape':<28}  {'Method':<18}  {'Median(ms)':>10}  {'p99(ms)':>9}  {'TFLOPS':>8}  {'VRAM Δ(MB)':>10}  {'vs baseline':>11}"
    ...
    for M, K, N, label in SHAPES:
        ...
        fp8_lin = Fp8AmpereLinear.from_fp8_weight(weight_fp8_u8, weight_scale, bias=None).to(device)

        results = {}

        # ---- baseline ----
        r = bench_baseline(x_bf16, weight_fp8_u8.clone())
        results["baseline"] = r

        # ---- fp8 hybrid (forward path) - use fresh module ----
        fp8_lin_hybrid = Fp8AmpereLinear.from_fp8_weight(weight_fp8_u8.clone(), weight_scale.clone(), bias=None).to(device)
        r = bench_fp8_hybrid(fp8_lin_hybrid, x_bf16)
        results["fp8_hybrid"] = r
        ...
        for method, r in results.items():
            ...
            if csv_mode:
                print(f"{label},{M},{K_aligned},{N_aligned},{method},{med:.4f},{p99:.4f},{tf:.3f},{vram:.1f},{speedup:.3f}")
            else:
                print(f"{label:<28}  {method:<18}  {med:>10.3f}  {p99:>9.3f}  {tf:>8.3f}  {vram:>10.1f}  {speedup:>10.2f}×")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--shape", nargs=3, type=int, metavar=("M", "K", "N"), help="Benchmark a single shape instead of the full suite")
    args = parser.parse_args()
    if args.shape:
        M, K, N = args.shape
        SHAPES[:] = [(M, K, N, f"custom ({M}×{K}×{N})")]
