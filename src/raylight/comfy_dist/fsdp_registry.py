...
class FSDPShardRegistry:
    ...
    @classmethod
    def wrap(cls, model, fsdp_state_dict=None, cpu_offload=False, patcher=None):
        ...
        for registered_cls, shard_func in cls._REGISTRY.items():
            if isinstance(model, registered_cls):
                ...
                return shard_func(model, fsdp_state_dict, cpu_offload, patcher=patcher)
        
        raise ValueError(f"No FSDP handler found for model type: {type(model)}")
...
if hasattr(model_base, "Flux"):
    @FSDPShardRegistry.register(model_base.Flux)
    def _wrap_flux(model, sd, cpu_offload, patcher=None):
        return flux_shard(model, sd, cpu_offload, patcher=patcher)

if hasattr(model_base, "WAN21"):
    @FSDPShardRegistry.register(model_base.WAN21)
    @FSDPShardRegistry.register(model_base.WAN22)
    def _wrap_wan(model, sd, cpu_offload, patcher=None):
        return wan_shard(model, sd, cpu_offload, patcher=patcher)

if hasattr(model_base, "ChromaRadiance"):
    @FSDPShardRegistry.register(model_base.ChromaRadiance)
    def _wrap_chroma_radiance(model, sd, cpu_offload, patcher=None):
...
if hasattr(model_base, "Chroma"):
    ...
    def _wrap_chroma(model, sd, cpu_offload, patcher=None):
